...
func CheckTCP(ctx context.Context, dep interface{}, _ *http.Client) error {
	...
	slog.Debug("Starting TCP dependency check", "target", target.Target)

	// First check if we can resolve the hostname
	host, port, err := net.SplitHostPort(target.Target)
	if err != nil {
		slog.Debug("Failed to split host:port", "target", target.Target, "error", err)
	...
	slog.Debug("Attempting TCP connection", "target", target.Target)
	...
	conn, err := dialer.DialContext(connCtx, "tcp", target.Target)
	if err != nil {
		slog.Debug("TCP connection failed", "target", target.Target, "error", err)
	...
	slog.Debug("TCP connection established", "target", target.Target,
	...
	if err != nil {
		slog.Debug("Failed to write probe to connection", "target", target.Target, "error", err)
	...
	if err != nil {
		if errors.Is(err, io.EOF) {
			...
			slog.Debug("Connection closed by server after probe (EOF)", "target", target.Target)
		} else if os.IsTimeout(err) {
			// In Docker environment, we need to be super strict about timeouts
			// if we already confirmed DNS resolution
			slog.Debug("Connection read timed out - marking as failure", "target", target.Target)
			return fmt.Errorf("connection read timed out: %w", err)
		} else {
			// Other network errors are considered failures
			slog.Debug("Connection read failed", "target", target.Target, "error", err)
			return fmt.Errorf("connection read failed: %w", err)
		}
	} else {
		responseData := string(buffer[:n])
		slog.Debug("Received response from server", "target", target.Target,
		...
		if strings.Contains(responseData, "302 Found") &&
			...
			slog.Debug("Detected Docker network placeholder response - rejecting", "target", target.Target)
			return fmt.Errorf("connection responded with Docker network placeholder")
		}
	}

	slog.Debug("TCP dependency check completed successfully", "target", target.Target)
	return nil
}
...
func CheckUDP(ctx context.Context, dep interface{}, _ *http.Client) error {
	...
	dialer := net.Dialer{}
	conn, err := dialer.DialContext(ctx, "udp", target.Target)
	if err != nil {
		return err
	}
	return conn.Close()
...
func CheckHTTP(ctx context.Context, dep interface{}, client *http.Client) error {
	...
	slog.Debug("Starting HTTP dependency check", "target", depInfo.Target, "type", depInfo.Type)
	...
	reqUrl := depInfo.Target
	if !strings.HasPrefix(reqUrl, "http://") && !strings.HasPrefix(reqUrl, "https://") {
		if depInfo.Type == "http" || depInfo.Type == "https" {
			reqUrl = fmt.Sprintf("%s://%s", depInfo.Type, depInfo.Target)
			slog.Debug("Prefixed URL with scheme", "original", depInfo.Target, "result", reqUrl)
		} else {
			return fmt.Errorf("invalid dependency type '%s' for http check on target '%s'", depInfo.Type, depInfo.Target)
		}
	}

	// Explicitly enforce HTTP if specified as the dependency type
	if depInfo.Type == "http" && strings.HasPrefix(reqUrl, "https://") {
...
func CheckExec(ctx context.Context, dep interface{}, _ *http.Client) error {
	...
	defer cancel()
	cmd := exec.CommandContext(cmdCtx, depInfo.Target, depInfo.Args...)
